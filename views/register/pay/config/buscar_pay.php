        ...
        <div class="row container-fluid d-flex px-1">
            <div class="col-xl-5">
                <div class="control container">
                    <center>
                        <h2><b> Búsqueda de Estudiante(s) a Pagar</b></h2>
                    </center>
                </div>
            </div>
            <div class="col-xl-2">
                ...
                <div class="control">
                    <div class="row">
                        <div class="col-md-auto" style="display: flex; align-content: stretch; justify-content: space-evenly;">
                            <form class="control-search" action="pago-taller.php?cedula=<?= $date['id_taller'] ?>" method=post>
<?php
$query_taller = mysqli_query($conexion,"SELECT id_taller FROM taller
where id_taller='$id_taller';");
$resultado = mysqli_num_rows($query_taller);
if($resultado > 0) {
    while ($date = mysqli_fetch_array($query_taller)) { ?>
    <input class="form-control" type="hidden" name="id_taller" value="<?php echo $date['id_taller'] ?>"> 
    <input class="searchText" type="search" name="search" placeholder="Buscar Pago" style="width: 190px;">
...
foreach ($id_est_from_taller as $id_est) {
    ...
    $result = mysqli_query($conexion, "SELECT estudiante.id_est, estudiante.cedula, estudiante.primer_nombre, estudiante.primer_apellido 
    FROM estudiante, cursa
    where cursa.id_taller=$id_est
    and cedula like '%".$buscar."%' or primer_nombre like '%".$buscar."%' or 
                primer_apellido like '%".$buscar."%';");
                if($result){
